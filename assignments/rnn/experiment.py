...
class Experiment:
    ...
    @classmethod
    def run(cls, tweets: Tweets, hidden_dim=100, num_tweets=100_000):
        ...
        for tweet_idx, tweet in zip(trange(num_tweets), itertools.cycle(tweets)):
            if tweet_idx % 10_000 == 0:
                samples[tweet_idx] = synthesize(
                    ada_grad.network, training_tweets=tweets, length=200
                )

            last_state = ada_grad.network.initial_state
            for fragment in tweet.fragments(length=24):
                states, outputs = ada_grad.network.run(
                    initial_state=last_state, tweet=fragment
                )
                losses[tweet_idx] = ada_grad.network.loss(
                    outputs=outputs, targets=fragment.targets
                )
                gradients = ada_grad.network.run_backward(
                    states=states, outputs=outputs, tweet=fragment
                )
    ...
    def loss_plot(self):
        ...
        return go.Figure(
            layout=dict(
                ...
                xaxis_title="Number of processed tweets",
